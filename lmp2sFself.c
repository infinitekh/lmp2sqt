 ...
 * 	NameI 	(nBuffDiffuse), // number of simul. time seq
 * 	NameI 	(nValDiffuse),   // number of time seq
 */
	NameR    (kVal),
	NameR    (deltaT),
	NameI   		(limitCorrAv),
 ...
 */
	...
	malloc_stream(&stream, n_snap);
	for ( int nAtom=0; nAtom <maxAtom; nAtom++) {
		dump_stream(&stream, fp, n_snap, maxAtom, 1, 1);

		for( int t0=0; t0< n_snap-nValCorr; t0++) {
			for( int Dt=1; Dt< nValCorr; Dt++) {
				Dx = stream.x[t0+Dt] - stream.x[t0];
				Dy = stream.y[t0+Dt] - stream.y[t0];
				Dz = stream.z[t0+Dt] - stream.z[t0];
...
void InitDiffuse () {
	int nb;
	for (nb = 0; nb < nBuffDiffuse; nb ++)
		tBuf[nb].countDiff = - nb * nValDiffuse / nBuffDiffuse;
	...
	for (nb = 0; nb < nBuffDiffuse; nb ++) {
		...
		if (tBuf[nb].countDiff >= 0) {
			...
			for (n=0; n<nCol; n++) {
				...
				tBuf[nb].rrDiffuse[ni] += dr.x*dr.x+dr.y*dr.y+dr.z*dr.z;
			}
		}
		++ tBuf[nb].countDiff;
	}
