...
int main(int argc, char** argv) {
	...
	real * r_done_works;
	int * i_done_works;
	AllocMem(threadSqt,nthreads,MakeSqtThread);
	AllocMem(r_done_works,nthreads,real);
	AllocMem(i_done_works,nthreads,int);

	for ( int tID =0; tID < nthreads; tID++) {
		threadSqt[tID].flag_alloc = 0;
		threadSqt[tID].flag_alloc_more = 0;
		threadSqt[tID].id = tID;
		i_done_works[tID]= 0;
	}
	...
	for( opt_num = 1;   opt_num < argc; opt_num++)  {
		...
		MakeSqtThread* threads_sqt = & threadSqt[threadID];
		InitSpacetimeCorr(threads_sqt);
		/*!
		...
		for (int ns = 0 ; ns < num_data[opt_num] ; ns++ ) {
			...
			if (snap == NULL)
				break;
			threads_sqt->snap = snap;
			EvalSpacetimeCorr(threads_sqt);
...
void AccumSpacetimeCorr (MakeSqtThread* threads_sqt) // __thread_safe__
	/*!
	 *  \brief  계산된 현재 시간의 SpaceTime correlation을 누적한다. 
	 */
{
	int k,  nb, nr, n, nt;
	TBuf* tBuf = threads_sqt->tBuf;
	TBuf* pt;
...
void InitSpacetimeCorr (MakeSqtThread* threads_sqt)
	/*!
	 *  \brief  프로그램 초기에 시간 평균을 낼 수 있도록 index를 부여하는 과정
	 */
{
	if (threads_sqt->flag_alloc == 0 ) {
		/* 		omp_set_lock(&write_lock);
		 */
		AllocArray(threads_sqt);
		threads_sqt->flag_alloc = 1;
		flag_global_alloc =1;
		AllocMemCheck();
		/* 		omp_unset_lock(&write_lock);
		 */
...
void ZeroOneTimeCorr(MakeSqtThread* threads_sqt)
{
	TBuf* tBuf = threads_sqt->tBuf;
...
void EvalOneTimeSumVR(MakeSqtThread* threads_sqt) 
{
	Rank2R3 VR;  
	VecR3  vecr3,vel;
	atom* col_i;
	TBuf* tBuf = threads_sqt->tBuf;
	Snapshot* snap = threads_sqt->snap;
	for (int n=0; n<nPtls; n++) {
		col_i = &(snap->atoms[n]);
...
__global__ void EvalOneTimeKspace_1st(real* rho_s_q1_dev,real* r, real* coordi_list_dev, int nCSpatial,real kVal){
	/* 		for (int k = 0; k < N_AXIS; k ++) {   
	 */
	int k = threadIdx.x;
	real b,c,s,c0,c1,s1,c2,s2;
	real cx=coordi_list_dev[3*k];
	real cy=coordi_list_dev[3*k+1];
	real cz=coordi_list_dev[3*k+2];
	for (int m = 0; m < nCSpatial; m ++) {  
		int 	marker = (nCSpatial* DOF)*k + DOF*m;

		if (m == 0) {
			b = kVal * ( r[0]*cx + r[1]*cy + r[2]*cz);
			c = cos (b);
			s = sin (b);
			c0 = c;
		} else if (m == 1) {
		...
		rho_s_q1_dev [marker +0] = c;
		rho_s_q1_dev [marker +1] = s;
	}
}
void EvalOneTimeKspace(MakeSqtThread* threads_sqt)
{
	real r[3], v[3],mu[3];
	Snapshot* snap = threads_sqt->snap;
	TBuf* tBuf = threads_sqt->tBuf;
	real *	rho_q1  = tBuf->rho_q1 ;
	real *	rho_s_q1_host  = threads_sqt->rho_s_q1_host ;
	real *	rho_s_q1_dev  = threads_sqt->rho_s_q1_dev ;
	real **	rho_s_q1  = tBuf->rho_s_q1 ;
	real **	rho_d_q1 = tBuf->rho_d_q1;
	atom* col_i;
	real* dev_r;
	AllocMem_dev(dev_r, 3, real);
	/*real* tmp_data;
		AllocMem(tmp_data,nCSpatial*DOF,real);*/
	/*-----------------------------------------------------------------------------
	 *  Direct calculate  rho(q)
	 *
	 *  for device memory copy :
	 *  input : nPtls, snapshot full, kVal(const)
	 *  				N_AXIS coordi_list nCSpatial DOF
	 *-----------------------------------------------------------------------------*/
	// copy dev_rx ry rz vx vy vz mux muy muz
	// copy iint nptls kVal, NAxis coordi_list nCSpatial DOF
	for (int n=0; n<nPtls; n++) {
		col_i = &(snap->atoms[n]);

		r[0]  =  col_i->x;   r[1] = col_i->y;   r[2]  = col_i->z; 
		HANDLE_ERROR(			cudaMemcpy(dev_r,r,3*sizeof(real), cudaMemcpyHostToDevice));

		v[0]  =  col_i->vx;  v[1] = col_i->vy;  v[2]  = col_i->vz;
		mu[0] = col_i->mux; mu[1] = col_i->muy; mu[2] = col_i->muz;
		// 
		if (FLAG_CU) {
			//puts("Eval OneTimeKspace_dev");
			EvalOneTimeKspace_1st<<<1,N_AXIS>>>(rho_s_q1_dev,
					dev_r, coordi_list_dev, 
					nCSpatial,kVal);
			//HANDLE_ERROR(			cudaMemcpy(rho_s_q1_host,rho_s_q1_dev,FDOF*nCSpatial*sizeof(real), cudaMemcpyDeviceToHost));
			//			puts("echo");
			long tmpsize= FDOF*nCSpatial*sizeof(real);
			//printf("cudaMemcpy (tmpdata,rhos,%d, DeviceToHost)\n ", tmpsize);
			HANDLE_ERROR(		cudaMemcpy(rho_s_q1_host,rho_s_q1_dev,tmpsize, cudaMemcpyDeviceToHost));

		} else {
#pragma acc parallel loop 
			for (int k = 0; k < N_AXIS; k ++) {   
				real b,c,s,c0,c1,s1,c2,s2;
				real cx=coordi_list[3*k];
				real cy=coordi_list[3*k+1];
				real cz=coordi_list[3*k+2];
				for (int m = 0; m < nCSpatial; m ++) {  
					int 	marker = (nCSpatial* DOF)*k + DOF*m;
					...
					s = sin( (m+1)*b);
#endif
					rho_s_q1_host[marker +0] = c;
					rho_s_q1_host[marker +1] = s;
				}// loop spatial slice
			}  // for DIMEN
		}

		/*		for(int nk=0; nk< FDOF * nCSpatial; nk++ ) {
					printf("rho_q[%9d] = %.6f = %8.4e - %8.4e\n", nk,
					tmp_data[nk]-rho_s_q1_host[nk], 
					tmp_data[nk],rho_s_q1_host[nk]);
					}*/
		...
		for(int nk=0; nk< FDOF * nCSpatial; nk++ ) {
			rho_q1 [ nk] += rho_s_q1_host[nk];
		}
		if ( flagSelf ) {
			memcpy(rho_s_q1[n], rho_s_q1_host, sizeof(real)*FDOF*nCSpatial);
		}
	} /* for loop : n<nPtls */ // second parallel zone
...
void EvalOneTimeCorr(MakeSqtThread* threads_sqt)
	/*!
	 * 
	 ...
	 */
{
	...
	ZeroOneTimeCorr(threads_sqt);

	EvalOneTimeSumVR(threads_sqt);
	EvalOneTimeKspace(threads_sqt);
}

void SetWaitedTimeCorr(MakeSqtThread* threads_sqt, TBuf* tBuf_tw)
{
	/*-----------------------------------------------
	 *   t_w information 
	 *-----------------------------------------------*/
...
void InitTwoTimeCorr (MakeSqtThread* threads_sqt, TBuf* tBuf_tw, int subtime)
{
	/*------------------------------
	 *  Zero initializing
	 *-----------------------------*/
...
void EvalTwoTimeEach(MakeSqtThread* threads_sqt, TBuf* tBuf_tw, int subtime)
{
	Snapshot* snap = threads_sqt->snap;
	for (int n=0; n<nPtls; n++) {
		VecR3 dr;
...
void EvalTwoTimeCollective(MakeSqtThread* threads_sqt, TBuf* tBuf_tw, int subtime)
{
	Rank2R3 subVR,sqVR;
	TBuf* tBuf = threads_sqt->tBuf;
...
__global__ void  EvalTwoTimeKspace_2nd (
		real* rho_s_q1_dev,real* org_rho_s_q1_dev,
		real* rho_d_q1_dev,real* org_rho_d_q1_dev,
		real* F_s_qq2_sub_dev, real* F_d_qq2_sub_dev,
		int nPtls,int nCSpatial,int pitch) 
{
	int nk = blockIdx.x;
	int axis_b = threadIdx.x;
	...
	w = 1.;
	nav = avMarker;
	// cos(q*1r(t)) cos(q*r(t_w) +sin sin
#define GetR2( A, row, column) \
	 (*( (real *)((char*)(A) + row * pitch) + column))

	for (int n=0; n<nPtls; n++) {
		F_s_qq2_sub_dev[nav] +=
			w * ( GetR2(rho_s_q1_dev,n,ncos) * GetR2(org_rho_s_q1_dev,n,ncos) +
					GetR2(rho_s_q1_dev,n,nsin) * GetR2(org_rho_s_q1_dev,n,nsin));
		F_d_qq2_sub_dev[nav] +=
			w * (GetR2(rho_d_q1_dev,n,ncos) * GetR2(org_rho_s_q1_dev,n,ncos) +
					GetR2(rho_d_q1_dev,n,nsin) * GetR2(org_rho_s_q1_dev,n,nsin))+
			w * (GetR2(rho_s_q1_dev,n,ncos) * GetR2(org_rho_d_q1_dev,n,ncos) +
					GetR2(rho_s_q1_dev,n,nsin) * GetR2(org_rho_d_q1_dev,n,nsin));
	}
	//}

}
__global__ void  EvalTwoTimeKspace_1st (
		real* rho_q1_dev, real* org_rho_q1_dev,
		real* F_qq2_sub_dev,
		int nPtls,int nCSpatial) 
{
	int nk = blockIdx.x;
	int axis_b = threadIdx.x;
	const int avMarker = nk*AVDOF;
	...
	F_qq2_sub_dev[nav] +=
		w * (rho_q1_dev[ncos] * org_rho_q1_dev[ncos] +
				rho_q1_dev[nsin] * org_rho_q1_dev[nsin]);
	//			}  // for nc, 
}
__global__ void kernel_zero_1d (real* a)
{
	int id = threadIdx.x + blockIdx.x * blockDim.x;
	
	a[id] = 0;
}
__global__ void kernel_zero_2d (real* a, int pitch)
{
	GetR2( a, blockIdx.x, threadIdx.x) = 0;
}

void EvalTwoTimeKSpace(MakeSqtThread* threads_sqt, TBuf* tBuf_tw, int subtime)
{
	TBuf* tBuf = threads_sqt->tBuf;
	real *	rho_q1  = tBuf->rho_q1 ;
	real **	rho_s_q1  = tBuf->rho_s_q1 ;
	...
	real *	org_rho_q1  = tBuf_tw->rho_q1 ;
	real *	org_rho_s_q1  = tBuf_tw->rho_s_q1[0] ;
	real *	org_rho_d_q1  = tBuf_tw->rho_d_q1[0] ;

	real *	rho_q1_dev  = threads_sqt->rho_q1_dev ;
	real *	org_rho_q1_dev  = threads_sqt->org_rho_q1_dev ;

	real *	rho_s_q1_dev  = threads_sqt->rho_s_q1_dev ;
	real *	org_rho_s_q1_dev  = threads_sqt->org_rho_s_q1_dev ;
	real *	rho_d_q1_dev  = threads_sqt->rho_d_q1_dev ;
	real *	org_rho_d_q1_dev  = threads_sqt->org_rho_d_q1_dev ;

	real *	F_qq2_sub_host  = threads_sqt->F_qq2_sub_host ;
	real *	F_qq2_sub_dev  = threads_sqt->F_qq2_sub_dev ;
	real *  F_s_qq2_sub_dev  = threads_sqt->F_s_qq2_sub_dev ;
	real *	F_d_qq2_sub_dev  = threads_sqt->F_d_qq2_sub_dev ;
	size_t pitch1 = threads_sqt->pitch1;
	size_t pitch1_host = threads_sqt->pitch1_host;
	size_t height1 = threads_sqt->height1;


	if (FLAG_CU) {
		size_t size_of_rho_q1 = FDOF*nCSpatial*sizeof(real);
		//HANDLE_ERROR(	cudaMemcpy(rho_q1_dev,rho_q1_dev,size_of_rho_q1, cudaMemcpyHostToDevice));
		HANDLE_ERROR(	cudaMemcpy(org_rho_q1_dev,org_rho_q1,size_of_rho_q1, cudaMemcpyHostToDevice));
		size_t nav_length = AVDOF * nCSpatial;
		size_t size_of_F_qq2_sub_host = nav_length * sizeof(real);
/*		for ( int nav = 0; nav < nav_length; nav += 1 ) {
			F_qq2_sub_host  [nav] = 0;
		}
		HANDLE_ERROR(	cudaMemcpy(F_qq2_sub_dev,F_qq2_sub_host,size_of_F_qq2_sub_host, cudaMemcpyHostToDevice));
 */
		kernel_zero_1d<<<nCSpatial, AVDOF>>> ( F_qq2_sub_dev );


		EvalTwoTimeKspace_1st <<<nCSpatial ,N_AXIS>>> (
				rho_q1_dev, org_rho_q1_dev,
				F_qq2_sub_dev, 
				nPtls,nCSpatial) ;

		HANDLE_ERROR(	cudaMemcpy(F_qq2_sub_host,F_qq2_sub_dev,size_of_F_qq2_sub_host, cudaMemcpyDeviceToHost));
		for ( int nav = 0; nav < nav_length; nav += 1 ) {
			tBuf_tw->F_qq2[nav][subtime] 	 += F_qq2_sub_host  [nav] ;
		}

		//puts("Eval OneTimeKspace_dev");
		if (flagSelf ) {
			kernel_zero_1d<<<nCSpatial, AVDOF>>> ( F_s_qq2_sub_dev );
			kernel_zero_1d<<<nCSpatial, AVDOF>>> ( F_d_qq2_sub_dev );

			HANDLE_ERROR( cudaMemcpy2D( org_rho_d_q1_dev,pitch1, 
					org_rho_d_q1, pitch1_host, pitch1_host,height1	, cudaMemcpyHostToDevice));
			HANDLE_ERROR( cudaMemcpy2D( org_rho_s_q1_dev,pitch1, 
					org_rho_s_q1, pitch1_host, pitch1_host,height1	, cudaMemcpyHostToDevice));
			EvalTwoTimeKspace_2nd <<<1,1 >>> (
					rho_s_q1_dev, org_rho_s_q1_dev,
					rho_d_q1_dev, org_rho_d_q1_dev,
					F_s_qq2_sub_dev, F_d_qq2_sub_dev,
					nPtls,nCSpatial,pitch1) ;
...
void EvalTwoTimeCorr(MakeSqtThread* threads_sqt, TBuf* tBuf_tw, int subtime)
{
	InitTwoTimeCorr(threads_sqt, tBuf_tw, subtime);
	EvalTwoTimeEach(threads_sqt, tBuf_tw, subtime);
	EvalTwoTimeCollective(threads_sqt, tBuf_tw, subtime);
	EvalTwoTimeKSpace(threads_sqt, tBuf_tw, subtime);
}
void EvalSpacetimeCorr(MakeSqtThread* threads_sqt)
	/*!
	 * 
	 ...
	 */
{
	...
	g_Vol  = L*L*L;
	nPtls = snap->n_atoms;
	if (threads_sqt->flag_alloc_more ==0 ) {
		/* 		omp_set_lock(&write_lock);
		 */
		Alloc_more(threads_sqt);
		threads_sqt->flag_alloc_more =1;
	...
	EvalOneTimeCorr(threads_sqt);

	// End Calculate Current time value
	// Begin Two time corrlation function
	for (int nb = 0; nb < nCBuffer; nb ++) {
		if (tBuf[nb].count == 0) {
			SetWaitedTimeCorr(threads_sqt, &tBuf[nb]);
		}     // End   buffer count ==0

		if (tBuf[nb].count >= 0) {
			EvalTwoTimeCorr(threads_sqt,&tBuf[nb],tBuf[nb].count);
		}                        // End buffer count >=0
		++ tBuf[nb].count;
	}
	AccumSpacetimeCorr (threads_sqt);
}
void AllocMemCheck ()
{
	if (ErrorAllocMem == 1) {
...
void AllocArray (MakeSqtThread* threads_sqt)
	/*!
	 ...
	 */
{
	...
	if (FLAG_CU ) {
		AllocMem_dev(coordi_list_dev, sizeof(coordi_list)/sizeof(real), real);
		HANDLE_ERROR(cudaMemcpy(coordi_list_dev,coordi_list,sizeof(coordi_list), cudaMemcpyHostToDevice));
	}
	...
	if ( flag_global_alloc ==0 ) {
		if( flagSelf ) {
			...
			AllocMem_dev (threads_sqt->F_s_qq2_sub_dev, AVDOF * nCSpatial,  real);
			AllocMem_dev (threads_sqt->F_d_qq2_sub_dev, AVDOF * nCSpatial,  real);
		}
	...
	AllocMem (threads_sqt->tBuf, nCBuffer, TBuf);
	TBuf* tBuf = threads_sqt->tBuf;
	...
	for (nb = 0; nb < nCBuffer; nb ++) {
		...
		AllocMem_dev (threads_sqt->F_qq2_sub_host, AVDOF * nCSpatial,  real);
		AllocMem_dev (threads_sqt->F_qq2_sub_dev, AVDOF * nCSpatial,  real);
	}
	/*!
	 *  \brief  Memory for Green-Kubo formula
	 */
	// AllocArray for Diffuse ()
	if (flag_global_alloc ==0 && threads_sqt->id == 0 ) {
...
void Alloc_more (MakeSqtThread* threads_sqt) 
{
	/*!
	 *  \brief  Alloc_more 
	...
	AllocMem (threads_sqt->rho_s_q1_host, FDOF * nCSpatial, real);
	size_t length_qmem = FDOF*nCSpatial;

	if (FLAG_CU ) {
		AllocMem_dev (threads_sqt->rho_s_q1_dev, length_qmem, real);

		if (flagSelf ) {
			AllocMemPitch_dev( threads_sqt->rho_s_q1_dev, threads_sqt->pitch1, nPtls, length_qmem, real);
			AllocMemPitch_dev( threads_sqt->rho_d_q1_dev, threads_sqt->pitch1, nPtls, length_qmem, real);
			AllocMemPitch_dev( threads_sqt->org_rho_s_q1_dev, threads_sqt->pitch1, nPtls, length_qmem, real);
			AllocMemPitch_dev( threads_sqt->org_rho_d_q1_dev, threads_sqt->pitch1, nPtls, length_qmem, real);
		}
	}
	...
	for (nb = 0; nb < nCBuffer; nb ++) {
		...
		if (flagSelf) {
			...
			threads_sqt->pitch1_host = sizeof(real)*FDOF*nCSpatial;
			threads_sqt->height1 =nPtls;
			
			AllocMem2 (tBuf[nb].org_rho_s_q1, nPtls, FDOF*nCSpatial,  real);
			AllocMem2 (tBuf[nb].org_rho_d_q1, nPtls, FDOF*nCSpatial,  real);
		}
	}
	fprintf(stderr, "Reserving memory on heap via AllocMem : %lld GB\n",  ll_mem_size/1000ll/1000ll/1000ll);
	if (flag_global_alloc ==0 && threads_sqt->id == 0 ) {
